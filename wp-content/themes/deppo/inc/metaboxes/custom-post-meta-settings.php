...
//add_action('admin_menu', 'tk_settings_page_meta_init');
$tabs = '';

function tk_settings_page_meta_init() {
    global $wp_version;
    $tk_theme_name = get_option('tk_theme_name');
    $pages = @get_all_pages();
    if ( version_compare( $wp_version, '3.4', '>=' ) ) {$theme_data = wp_get_theme(TEMPLATEPATH . '/style.css');}else{$theme_data = wp_get_theme(TEMPLATEPATH . '/style.css');}
    $settings_page = '';
    for ($i = 0; $i <= count($pages) - 1; $i++) {
        if ($i == 0) {
            $settings_page .= add_theme_page($pages[$i]['slug'], 'Themes Kingdom', 'edit_theme_options', $pages[$i]['slug'], 'tk_settings_page_meta', get_template_directory_uri().'/style/img/icon-kingdom.png');
            $settings_page .= add_theme_page($pages[0]['slug'], $pages[$i]['page_title'], $pages[$i]['menu_title'], 'edit_theme_options', $pages[$i]['slug'], 'tk_settings_page_meta');
        } else {
            $settings_page .= add_theme_page($pages[0]['slug'], $pages[$i]['page_title'], $pages[$i]['menu_title'], 'edit_theme_options', $pages[$i]['slug'], 'tk_settings_page_meta');
        }
    }
...
function tk_settings_page_meta($par) {
